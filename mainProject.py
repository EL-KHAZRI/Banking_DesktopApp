...
1. EL-KHAZRI HICHAM 
grazie mille to , merci beaucoup , thanks , chokran 
2. OPNAI CHATGPT
...
def transaction(identity, amount, choice):  # choice 1 for deposit; choice 2 for withdraw
    ...
    for line in customer_database:
        if identity == line.replace("\n", ""):
            ...
            if choice == 2 and balance - amount < 100:  # Minimum balance 10000
                return -1
            else:
                if choice == 1:
                    balance += amount
...
class welcomeScreen:
    def __init__(self, window=None):
        ...
        window.title("Welcome to Axentis BANK")
...
class deleteAdmin:
    ...
    def delete_admin(self, admin_id, password):
        if admin_id == "hicham" or admin_id == admin_idNO:
            Error(Toplevel(self.master))
...
class depositMoney:
    ...
    def submit(self, amount):
        ...
        if output == -1:
            ...
            output = str(amount) +"$"  + " deposited successfully.\nUpdated balance : " + str(output)
...
class withdrawMoney:
    ...
    def submit(self, amount):
        ...
        if output == -1:
            ...
            output = str(amount) +"$"+ " withdrawn successfully.\nUpdated balance : " + str(output)
            customerMenu.printMessage_outside(output)
            self.master.withdraw()
